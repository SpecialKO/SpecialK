...
// Helpers: UTF-8 <> wchar
IMGUI_API int           ImTextStrToUtf8              (      char*             buf,             int    buf_size,
...
IMGUI_API int           ImTextStrFromUtf8            (      ImWchar*          buf,             int    buf_size,
...
IMGUI_API int            ImFormatString               (         char* buf,                   size_t  buf_size,
                                                       const    char* fmt, ...         ) IM_FMTARGS (3);
IMGUI_API int            ImFormatStringV              (         char* buf,                   size_t  buf_size,
...
static inline ImVec2& operator/= (      ImVec2& lhs, const float   rhs) {  lhs.x /= rhs;   lhs.y /= rhs;   return lhs;  }
static inline ImVec4  operator+  (const ImVec4& lhs, const ImVec4& rhs) { return ImVec4 (lhs.x + rhs.x, lhs.y + rhs.y,
...
template<typename T> static inline T    ImClamp            (    T  v,   T  mn,
                                                                T  mx)          { return (v < mn) ? mn : (v > mx) ? mx : v; }
template<typename T> static inline T    ImLerp             (    T  a,   T  b,
                                                            float  t)           { return (T)(a + (b - a) * t); }
template<typename T> static inline void ImSwap             (    T& a,   T& b)   { T tmp = a; a = b; b = tmp; }
template<typename T> static inline T    ImAddClampOverflow (    T  a,   T  b,
                                                                T  mn,  T  mx)  { if (b < 0 && (a < mn - b)) return mn; if (b > 0 && (a > mx - b)) return mx; return a + b; }
template<typename T> static inline T    ImSubClampOverflow (    T  a,   T  b,
                                                                T  mn,  T  mx)  { if (b > 0 && (a < mn + b)) return mn; if (b < 0 && (a > mx + b)) return mx; return a - b; }
// - Misc maths helpers
...
static inline constexpr /* Kaldaien added constexpr (12/27/21)*/
              float  ImFloor       (float f) { return (float)(int)f; }
...
struct IMGUI_API ImPool
{
  ...
  void      Clear         (void)                      { for (int n = 0; n < Map.Data.Size; n++) { int idx = Map.Data[n].val_i; if (idx != -1) Data[idx].~T (); } Map.Clear (); Data.clear (); FreeIdx = 0; }
  T*        Add           (void)                      { int idx = FreeIdx; if (idx == Data.Size) { Data.resize (Data.Size + 1); FreeIdx++; } else { FreeIdx = *(int*)& Data[idx]; } IM_PLACEMENT_NEW (&Data[idx]) T (); return &Data[idx]; }
  void      Remove        (      ImGuiID   key,
                           const T*          p)       { Remove (key, GetIndex (p)); }
  void      Remove        (      ImGuiID   key,
                                 ImPoolIdx idx)       { Data[idx].~T (); *(int*)& Data[idx] = FreeIdx; FreeIdx = idx; Map.SetInt (key, -1); }
  void      Reserve       (      int  capacity)       { Data.reserve (capacity); Map.Data.reserve (capacity); }
  int       GetSize       (void)                const { return Data.Size; }
};
...
{
  ImGuiButtonFlags_None                  = ~std::numeric_limits <unsigned int>::max (),
...
{
  ImGuiSliderFlags_None     = ~std::numeric_limits <unsigned int>::max (),
  ImGuiSliderFlags_Vertical = 1 << 0
};

enum ImGuiDragFlags_
{
  ImGuiDragFlags_None     = ~std::numeric_limits <unsigned int>::max (),
...
{
  // Default: 0
  ImGuiColumnsFlags_None                   = ~std::numeric_limits <unsigned int>::max (),
...
{
  ImGuiSeparatorFlags_None       = ~std::numeric_limits <unsigned int>::max (),
...
{
  ImGuiItemFlags_Default_                 = ~std::numeric_limits <unsigned int>::max (),
...
{
  ImGuiItemStatusFlags_None             = ~std::numeric_limits <unsigned int>::max (),
...
{
  ImGuiTextFlags_None                       = ~std::numeric_limits <unsigned int>::max (),
...
{
  ImGuiLayoutType_Horizontal = ~std::numeric_limits <unsigned int>::max (),
  ImGuiLayoutType_Vertical   = 1
};

enum ImGuiLogType
{
  ImGuiLogType_None = ~std::numeric_limits <unsigned int>::max (),
  ImGuiLogType_TTY,
  ImGuiLogType_File,
  ImGuiLogType_Buffer,
...
{
  ImGuiInputSource_None = ~std::numeric_limits <unsigned int>::max (),
...
{
  ImGuiNavHighlightFlags_None        = ~std::numeric_limits <unsigned int>::max (),
...
{
  ImGuiNavDirSourceFlags_None      = ~std::numeric_limits <unsigned int>::max (),
...
{
  ImGuiNavMoveFlags_None                = ~std::numeric_limits <unsigned int>::max (),
...
struct IMGUI_API ImRect
{
  ...
  ImRect (void) :  Min (FLT_MAX, FLT_MAX),
                                   Max (-FLT_MAX, -FLT_MAX) { }
  ...
  ImRect (       float  x1, float y1,
                 float  x2, float y2) :
                   Min (x1, y1),   Max (x2, y2)             { }

  ImVec2      GetCenter    (void)            const { return ImVec2 ((Min.x + Max.x) * 0.5f, (Min.y + Max.y) * 0.5f); }
  ImVec2      GetSize      (void)            const { return ImVec2 (Max.x - Min.x, Max.y - Min.y); }
  float       GetWidth     (void)            const { return Max.x - Min.x; }
  float       GetHeight    (void)            const { return Max.y - Min.y; }
  ImVec2      GetTL        (void)            const { return Min; }                   // Top-left
  ImVec2      GetTR        (void)            const { return ImVec2 (Max.x, Min.y); }  // Top-right
  ImVec2      GetBL        (void)            const { return ImVec2 (Min.x, Max.y); }  // Bottom-left
  ImVec2      GetBR        (void)            const { return Max; }                   // Bottom-right
  ...
  void        Floor        (void)                  { Min.x = (float)(int)Min.x; Min.y = (float)(int)Min.y; Max.x = (float)(int)Max.x; Max.y = (float)(int)Max.y; }
  bool        IsInverted   (void)            const { return Min.x > Max.x || Min.y > Max.y; }
};
...
struct ImGuiStyleMod
{
  ...
  union { int   BackupInt   [2];
          float BackupFloat [2]; };

  ImGuiStyleMod (ImGuiStyleVar idx, int    v) { VarIdx = idx; BackupInt   [0] = v;   }
  ImGuiStyleMod (ImGuiStyleVar idx, float  v) { VarIdx = idx; BackupFloat [0] = v;   }
  ImGuiStyleMod (ImGuiStyleVar idx, ImVec2 v) { VarIdx = idx; BackupFloat [0] = v.x;
                                                              BackupFloat [1] = v.y; }
};
...
struct IMGUI_API ImGuiInputTextState
{
  ...
  ImGuiInputTextState    (void)       { memset (this, 0, sizeof (*this)); }
  void ClearFreeMemory   (void)       { TextW.clear (); TextA.clear (); InitialTextA.clear (); }
  void CursorAnimReset   (void)       { CursorAnim = -0.30f; }                                   // After a user-input the cursor stays on for a while without blinking
  void CursorClamp       (void)       { Stb.cursor = ImMin (Stb.cursor, CurLenW); Stb.select_start = ImMin (Stb.select_start, CurLenW); Stb.select_end = ImMin (Stb.select_end, CurLenW); }
  bool HasSelection      (void) const { return Stb.select_start != Stb.select_end; }
  void ClearSelection    (void)       { Stb.select_start = Stb.select_end = Stb.cursor; }
  void SelectAll         (void)       { Stb.select_start = 0; Stb.cursor = Stb.select_end = CurLenW; Stb.has_preferred_x = 0; }
  int  GetUndoAvailCount (void) const { return Stb.undostate.undo_point; }
  int  GetRedoAvailCount (void) const { return STB_TEXTEDIT_UNDOSTATECOUNT - Stb.undostate.redo_point; }
...
struct ImGuiWindowSettings
{
  ...
  ImGuiWindowSettings (void) { Name = NULL; ID = 0; Pos = Size = ImVec2 (0, 0); Collapsed = false; }
};
...
struct ImGuiColumnData
{
  ...
  ImGuiColumnData (void) { OffsetNorm = OffsetNormBeforeResize = 0.0f; Flags = 0; }
};
...
struct ImDrawDataBuilder
{
  ...
  void Clear           (void) { for (int n = 0; n < IM_ARRAYSIZE (Layers); n++) Layers[n].resize (0); }
  void ClearFreeMemory (void) { for (int n = 0; n < IM_ARRAYSIZE (Layers); n++) Layers[n].clear (); }

  IMGUI_API void FlattenIntoSingleLayer (void);
};
...
namespace ImGui
{
  ...
  IMGUI_API void          RenderColorRectWithAlphaCheckerboard (ImVec2 p_min, ImVec2 p_max, const ImColor& fill_col, float grid_step, ImVec2 grid_off, float rounding = 0.0f, int rounding_corners_flags = ~0);
  IMGUI_API void          RenderArrow (ImVec2 pos, ImGuiDir dir, float scale = 1.0f);
  IMGUI_API void          RenderBullet (ImVec2 pos);
  IMGUI_API void          RenderCheckMark (ImVec2 pos, const ImColor& col, float sz);
  ...
  IMGUI_API void          RenderPixelEllipsis (ImDrawList* draw_list, ImVec2 pos, int count, const ImColor& col);
  ...
  IMGUI_API void          PlotCEx (ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, float saturation, float value, float avg, bool inverse);
