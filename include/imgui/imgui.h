...
struct ImDrawList
{
    ...
    IMGUI_API void  AddText(const ImVec2& pos, const ImVec4& col, const char* text_begin, const char* text_end = NULL);
    ...
    inline    void  PrimWriteVtx(const ImVec2& pos, const ImVec2& uv, ImU32 col)         { _VtxWritePtr->pos = pos; _VtxWritePtr->uv = uv; _VtxWritePtr->col = ImColor (col); _VtxWritePtr++; _VtxCurrentIdx++; }
    inline    void  PrimWriteVtx(const ImVec2& pos, const ImVec2& uv, const ImVec4& col) { _VtxWritePtr->pos = pos; _VtxWritePtr->uv = uv; _VtxWritePtr->col = ImColor (col); _VtxWritePtr++; _VtxCurrentIdx++; }
