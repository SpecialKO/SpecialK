...
SK_D3D11_IgnoreWrappedOrDeferred ( bool                 bWrapped,
                                   bool                 bDeferred,
                                   ID3D11DeviceContext* pDevCtx ) noexcept
{
...
struct d3d11_shader_tracking_s
{
  ...
  void use        ( ID3D11DeviceContext* pDevCtx ) noexcept;
  void use_cmdlist( ID3D11DeviceContext* pDevCtx ) noexcept; // Deferred draw, cannot be timed
  ...
  struct
  {
    ...
    bool get (ID3D11DeviceContext* pDevCtx) noexcept
    {
      return
        get (SK_D3D11_GetDeviceContextHandle (pDevCtx));
    }

    void set (int dev_idx, bool active) noexcept
    {
      assert (dev_idx < SK_D3D11_MAX_DEV_CONTEXTS);
    ...
    void set (ID3D11DeviceContext* pDevCtx, bool active) noexcept
    {
      set (
  ...
  void addClassInstance (ID3D11ClassInstance* pInstance) noexcept
  {
...
struct SK_D3D11_KnownShaders
{
  ...
  bool hasReShadeTriggers (void) noexcept
  {
    return
...
void SK_ImGui_QueueResetD3D11 (void) noexcept;
...
bool SK_D3D11_IsDirectCopyCompatible (DXGI_FORMAT src, DXGI_FORMAT dst) noexcept;
