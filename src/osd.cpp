  ...
  *pszOSD = '\0';
  ...
  if (config.fps.show)
  {
    ...
    extern std::wstring SK_RenderAPI;
                            ...
                            //SK_IsD3D11                (pApp->dwFlags);
                            //SK_IsOpenGL               (pApp->dwFlags);
    std::wstring api_name = SK_RenderAPI;//L"D3D11";//SK_GetAPINameFromOSDFlags (pApp->dwFlags);
    ...
    if (mean != INFINITY) {
      ...
      // No Frametime History
        ...
        OSD_PRINTF "  %-6ws :  %#4.01f FPS, %#13.01f ms",
          api_name.c_str (),
            // Cast to FP to avoid integer division by zero.
            1000.0f * 0.0f / 1.0f, 0.0f //(float)pApp->dwFrames / (float)(pApp->dwTime1 - pApp->dwTime0),
              //pApp->dwFrameTime / 1000.0f
        OSD_END
      }
  ...
  if (font_.cegui && geometry_)
  {
    ...
    while (line != nullptr) {
      ...
      font_.cegui->drawText (
        *geometry_,
          line,
            CEGUI::Vector2f (0.0f + 2.0f, baseline + 2.0f),
              nullptr,
                CEGUI::Colour ( 0,
                                  0,
                                    0 ),
