...
SK_TopLevelExceptionFilter ( _In_ struct _EXCEPTION_POINTERS *ExceptionInfo )
{
  ...
  SymSetOptions ( SYMOPT_CASE_INSENSITIVE | SYMOPT_ALLOW_ZERO_ADDRESS     | SYMOPT_LOAD_LINES |
                  SYMOPT_LOAD_ANYTHING    | SYMOPT_ALLOW_ABSOLUTE_SYMBOLS | SYMOPT_UNDNAME    |
                  SYMOPT_NO_PROMPTS       | SYMOPT_DEBUG );
...
#else
  DWORD64  ip;    UIntPtrToInt64 (ExceptionInfo->ContextRecord->Eip, (int64_t *)&ip);
#endif
...
SK_BypassSteamCrashHandler (void)
{
  if (! config.steam.silent)
  {
    ...
    HMODULE hMod = LoadLibraryExW_Original (wszSteamDLL, nullptr, DONT_RESOLVE_DLL_REFERENCES | LOAD_LIBRARY_AS_DATAFILE );
