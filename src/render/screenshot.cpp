...
bool
SK_AVIF_CopyToClipboard (const wchar_t *wszPathToImage)
{
  int clpSize = sizeof (DROPFILES);

  clpSize += sizeof (wchar_t) * static_cast <int> (wcslen ((wchar_t *)wszPathToImage) + 1); // + 1 => '\0'
  ...
  if (df != nullptr)
  {
    ...
    wcscpy ((wchar_t*)&df [1], (const wchar_t *)wszPathToImage);
...
static avifImageSetProfileICC_pfn     SK_avifImageSetProfileICC     = nullptr;
...
SK_Screenshot_SaveAVIF (DirectX::ScratchImage &src_image, const wchar_t *wszFilePath, uint16_t max_cll, uint16_t max_pall)
{
  ...
  SK_RunOnce (
  {
    ...
    if (hModAVIF != 0)
    {
      ...
      SK_avifImageSetProfileICC     = (avifImageSetProfileICC_pfn)    SK_GetProcAddress (hModAVIF, "avifImageSetProfileICC");
  ...
  if (rgbToYuvResult == AVIF_RESULT_OK)
  {
    ...
    if (encoder != nullptr && image != nullptr)
    {
      ...
      sk_avif_add_icc_to_image (image);
...
void
sk_avif_add_icc_to_image (avifImage* img)
{
