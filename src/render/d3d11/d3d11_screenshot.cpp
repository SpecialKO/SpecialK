                                    ...
                                    bool               purge)
{
  ...
  static concurrency::concurrent_queue <SK_D3D11_Screenshot *> screenshot_queue;
  static concurrency::concurrent_queue <SK_D3D11_Screenshot *> screenshot_write_queue;
              ...
              while (0 == ReadAcquire (&__SK_DLL_Ending))
              {
                ...
                { while (! images_to_write->try_pop (pFrameData))
                  ...
                  if (pFrameData != nullptr)
                  {
                    ...
                    if ( (pFrameData->AllowCopyToClipboard && (config.screenshots.copy_to_clipboard && rb.screenshot_mgr->getClipboardFormat () == SK_HDR_CLIPBOARD_FORMAT_AVIF)) || (pFrameData->AllowSaveToDisk && config.screenshots.png_compress && ((! config.screenshots.use_hdr_png) || (! hdr))))
                    { if (pFrameData->AllowCopyToClipboard &&  config.screenshots.copy_to_clipboard && rb.screenshot_mgr->getClipboardFormat () == SK_HDR_CLIPBOARD_FORMAT_AVIF && (! pFrameData->AllowSaveToDisk))
                      {
...
{
  if (     InterlockedCompareExchange (&__SK_D3D11_InitiateHudFreeShot, -1, -2) == -2);
  else if (InterlockedCompareExchange (&__SK_D3D11_InitiateHudFreeShot,  0, -1) == -1)
  {
    SK_D3D11_ShowGameHUD ();
  }
}
...
bool SK_Screenshot_D3D11_BeginFrame (void)
{
       ...
       ReadAcquire (&__SK_D3D11_InitiateHudFreeShot) != 0    )
  {
    ...
    if (InterlockedCompareExchange (&__SK_D3D11_InitiateHudFreeShot, -3, 1) == 1)
    {
      SK_D3D11_HideGameHUD ();
    ...
    else if (InterlockedCompareExchange (&__SK_D3D11_InitiateHudFreeShot, -2, -3) == -3)
    {
