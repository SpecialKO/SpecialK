  ...
  _In_opt_       ID3D11ShaderResourceView* const *ppShaderResourceViews )
{
  ...
  // ImGui gets to pass-through without invoking the hook
  if (((! config.render.dxgi.deferred_isolation) && This->GetType () == D3D11_DEVICE_CONTEXT_DEFERRED) || pTLS->imgui.drawing || (! SK_D3D11_EnableTracking))
  ...
  if (SUCCEEDED (hr))
  {
    ...
    if (SK_D3D11_IsStagingCacheable (rdim, pResource))
    {
      ...
      if (pTex != nullptr)
      {
        ...
        //dll_log.Log ( "Staging Map: Type=%x, BindFlags: %s, Res: %lux%lu",
  ...
  _In_ ID3D11Resource      *pSrcResource )
{
  ...
  // ImGui gets to pass-through without invoking the hook
  if (pTLS->imgui.drawing || (! (SK_D3D11_EnableTracking || config.textures.cache.allow_staging)))
  {
          ...
          if (SUCCEEDED (hrMap))
          {
            for ( auto& ovr : overrides )
            {
              if ( ovr.Slot == j && mapped_sub.pData != nullptr )
              {
                ...
                memcpy (pBase, ovr.Values, std::min (ovr.Size, 64ui32));
              }
            }
...
SK_D3D11_TexCacheCheckpoint (void)
{
      ...
      pDevice &&        SUCCEEDED (pDevice->QueryInterface <IDXGIDevice>  (&pDXGIDev)) && pDXGIDev != nullptr)
  {
    ...
    const int MAX_TEXTURES_PER_PASS = 192UL;
    ...
    if (dim == D3D11_RESOURCE_DIMENSION_TEXTURE2D)
    {
      ...
      if (pTex)
      {
        ...
        if (override)
        {
          ...
          if (newMipLevels != pDescOrig->Texture2D.MipLevels)
          {
                    ...
                    LPVOID                   lpCallerAddr)
{
  ...
  if (! bIgnoreThisUpload) bIgnoreThisUpload = SK_D3D11_IsTexInjectThread ();
  ...
  SK_D3D11_TextureResampler.processFinished (This, pDevCtx);
  ...
  if (list_dirty)
  {
...

    {
      ...
      for (auto& it : SK_D3D11_Textures.HashMap_2D)
      {
        ...
        {
          if ( SK_D3D11_Textures.TexRefs_2D.find (it2.second) != 
               SK_D3D11_Textures.TexRefs_2D.cend () )
          {
            list_entry_s entry = { };
      ...
      if (pDev != nullptr)
      {
        ...
        if (! entry.injected)
        {
          ...
          if (ImGui::Button ("  Generate Mipmaps  ###GenerateMipmaps"))
          {
            ...
            __stdcall
            SK_D3D11_MipmapCacheTexture2D ( _In_ ID3D11Texture2D* pTex, uint32_t crc32c );
            SK_D3D11_MipmapCacheTexture2D (pTex, entry.crc32c);
          }
        }
...
SK_LiveShaderClassView (sk_shader_class shader_type, bool& can_scroll)
{
                                               ...
                                               list->sel < (int)list->contents.size ())
  {
    ...
    {
      if (ImGui::Checkbox ("Use ComputeShader Anti-Stall For Buffer Injection", &__SKX_ComputeAntiStall))
      {
        //
      }
    }
...
public:
         ...
         )
      {
           ...
           )
        {
             ...
             )
          {
            ...
            {
              pImmediateCtx->CopyResource (pStagingBackbufferCopy, pBackbufferSurface);
                 ...
                 bool      Wait = false )
  {
    ...
    {
      const UINT BytesPerPel = DirectX::BitsPerPixel (framebuffer.NativeFormat) / 8UL;
...
concurrency::concurrent_queue <SK_D3D11_Screenshot> queued_screenshots;
...
void
SK_D3D11_ProcessScreenshots (void)
{
  if (! queued_screenshots.empty ())
  {
    ...
    if (InterlockedCompareExchangePointer (&hSignalScreenshot, 0, INVALID_HANDLE_VALUE) == INVALID_HANDLE_VALUE)
    {
      ...
      SK_Thread_Create ([](LPVOID) -> DWORD
      {
        ...
        while (! ReadAcquire (&__SK_DLL_Ending))
        {
          ...
          while (! queued_screenshots.empty ())
          {
            SK_D3D11_Screenshot pop_off (_invalid);

            if (queued_screenshots.try_pop (pop_off))
            {
              UINT     Width, Height;
              uint8_t* pData;
              ...
              if (pop_off.getData (&Width, &Height, &pData))
              {
                ...
                raw_img.format = DXGI_FORMAT_R8G8B8A8_UNORM;
                raw_img.width  = Width;
                ...
                wcsncpy     (wszAbsolutePathToScreenshot, SK_GetRootPath (), MAX_PATH * 2);
          ...
          while (! rejected_screenshots.empty ())
          {
            ...
            {
              queued_screenshots.push (push_back);
            }
          }
        }
