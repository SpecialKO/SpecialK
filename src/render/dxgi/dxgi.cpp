...
ImGui_DX11Startup ( IDXGISwapChain* pSwapChain )
{
  ...
  if ( SUCCEEDED (pSwapChain->GetDevice (IID_PPV_ARGS (&pD3D11Dev.p))) )
  {
    ...
    if ( pImmediateContext != nullptr )
    {
      ...
      return
        _d3d11_rbk->init ( pSwapChain, pD3D11Dev, pImmediateContext);
    }
  }
...
SK_DXGI_UpdateColorSpace (IDXGISwapChain3* This, DXGI_OUTPUT_DESC1 *outDesc = nullptr)
{
  if (This != nullptr)
  {
    ...
    if (outDesc != nullptr)
    {
      ...
      rb.scanout.dxgi_colorspace = outDesc->ColorSpace;
        ...
        if (sk::NVAPI::nv_hardware != false)
        {
          ...
          if ( NVAPI_OK ==
                 NvAPI_GetAssociatedNvidiaDisplayHandle (
                   SK_FormatString (R"(\\.\DISPLAY%lu)", devIdx).c_str (),
                     &nvDisplayHandle
                 ) &&
               NVAPI_OK ==
             ...
             )
          {
            ...
            {
              if ( SK_EDID_Parse ( edid.EDID_Data,
                                   edid.sizeofEDID )
                 )
              {
                nvSuppliedEDID = true;
...
SK_ImGui_DrawD3D12 (IDXGISwapChain* This)
{
  ...
  switch (swapDesc1.Format)
  {
    ...
    case DXGI_FORMAT_R16G16B16A16_FLOAT:
    {
      ...
      if (rb.scanout.dxgi_colorspace == DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_P709)
          rb.framebuffer_flags       |= SK_FRAMEBUFFER_FLAG_HDR;
    } break;

    case DXGI_FORMAT_R10G10B10A2_TYPELESS:
    case DXGI_FORMAT_R10G10B10A2_UNORM:
      if (rb.scanout.dxgi_colorspace == DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020)
          rb.framebuffer_flags       |= SK_FRAMEBUFFER_FLAG_HDR;
...
#define _SetupThreadContext()                                                   \
  ...
  if ( pDev != nullptr )
  {
    ...
    // Skip it, cause it's not initialized...
    //   maybe we'll catch initialization a frame later?
    //
    if ( _d3d11_rbk->frames_.empty () ||
         _d3d11_rbk->frames_ [0].pRenderOutput.p == nullptr )
    {
      if  (! _d3d11_rbk->init (This, pDev, rb.d3d11.immediate_ctx))
        return;
...
)
{
  ...
  if (process)//|| SK_GetFramesDrawn () < 2)
  {
    ...
    if ( pDev12 == nullptr && pDev == nullptr )
    {
  ...
  _In_ UINT            SwapChainFlags)
{
  ...
  if (FAILED (ret))
  {
    if (SK_IsDebuggerPresent ())
    {
      ...
      SK_DXGI_ReportLiveObjects ( rb.device );
    }
                      ...
                      _In_ const DXGI_MODE_DESC *pNewTargetParameters )
{
  ...
  _d3d11_rbk->release (This);
  _d3d12_rbk->release (This);
  //SK_CEGUI_QueueResetD3D12 (); // Prior to next present, reset the UI
...
static concurrency::concurrent_unordered_map <HWND, IUnknown*>           _recyclable_d3d11;
static concurrency::concurrent_unordered_map <HWND, IDXGISwapChain1*>    _recyclable_d3d12;
#else
...
SK_DXGI_GetCachedSwapChainForHwnd (HWND hWnd, IUnknown* pDevice)
{
  ...
  {
    pChain = (IDXGISwapChain1 *)_recyclable_d3d11 [hWnd];
  }

  else if ( _recyclable_d3d12.count (hWnd) &&
            _recyclable_d3d12.at    (hWnd) != nullptr )
  {
    SK_ReleaseAssert (_d3d12_rbk->_pSwapChain.IsEqualObject (_recyclable_d3d12 [hWnd]));

    _d3d12_rbk->release                (nullptr);
    _d3d12_rbk->_pCommandQueue.Release (       );

    while (SK_GetCurrentRenderBackend ().swapchain.p->Release () > 1)
      ;
    SK_GetCurrentRenderBackend ().releaseOwnedResources ();

    _recyclable_d3d12 [hWnd] = nullptr;
  }
#else
...
#ifdef __PAIR_DEVICE_TO_CHAIN
  if (pDevice != nullptr)
  _recyclable_d3d11 [hWnd] = pSwapChain;
  else
  _recyclable_d3d12 [hWnd] = pSwapChain;
#else
...
#else
#ifdef __PAIR_DEVICE_TO_CHAIN
  if (_recyclable_d3d11 [hWnd] != 0)
  {
    ret = 0;
    _recyclable_d3d11 [hWnd] = nullptr;
  }
...
//    return CreateSwapChainForHwnd_Original (This, pDevice, hWnd, pDesc, pFullscreenDesc, pRestrictToOutput, ppSwapChain);
  ...
  if (pCmdQueue != nullptr)
  {
    ...
    //
    SK_ComPtr <ID3D12Device>             pDev;
    pCmdQueue->GetDevice (IID_PPV_ARGS (&pDev.p));
  ...
  if (SK_DXGI_IsFlipModelSwapEffect (new_desc1.SwapEffect)) pCache =
  ...
  if ( SUCCEEDED (ret) )
  {
    ...
    if (SK_DXGI_IsFlipModelSwapEffect (new_desc1.SwapEffect))
    {
      ...
      if (pDev11.p != nullptr)
      {
        //SK_D3D11_MakeCachedDeviceAndSwapChainForHwnd
        SK_DXGI_MakeCachedSwapChainForHwnd
             ( (IDXGISwapChain1 *)*ppSwapChain,  hWnd, (ID3D11Device *)pDevice );
                                 (*ppSwapChain)->AddRef ();
      }

      // D3D12, half-heartedly cache this stuff.
      else
      {
        SK_DXGI_MakeCachedSwapChainForHwnd (
               *ppSwapChain, hWnd, nullptr );
              (*ppSwapChain)->AddRef ();
      }
    }
...
/////#endif
  ...
  if (! InterlockedCompareExchangeAcquire (&__hooked, TRUE, FALSE))
  {
    ...
    using CreateDXGIFactory_pfn =
      HRESULT (WINAPI*)(const IID&, void **);
    using D3D11CreateDeviceAndSwapChain_pfn =
    ...
    using D3D12CreateDevice_pfn =
      HRESULT (WINAPI*)(IUnknown*, D3D_FEATURE_LEVEL, const IID&, void **);

    auto CreateDXGIFactory = (CreateDXGIFactory_pfn) SK_GetProcAddress ( L"dxgi.dll",
                             "CreateDXGIFactory");
    if ( CreateDXGIFactory != nullptr &&
         GetModuleHandleEx ( GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT |
                             GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
                     reinterpret_cast <LPCWSTR> (CreateDXGIFactory), &hModDXGI ) != 0 )
    {
      ...
      try
      {
        ...
        //
        ThrowIfFailed (
          CreateDXGIFactory (IID_PPV_ARGS (&pFactory.p)));
        ThrowIfFailed (
          pFactory->EnumAdapters (0,       &pAdapter.p));

        SK_DXGI_HookFactory                (pFactory.p);

        // D3D12
        //
        try
        {
          SK_ComPtr <ID3D12Device>              pD3D12Device;
          ...
          auto D3D12CreateDevice = (D3D12CreateDevice_pfn) SK_GetProcAddress ( L"d3d12.dll",
                                   "D3D12CreateDevice");
          ...
          if ( config.apis.dxgi.d3d12.hook &&
              D3D12CreateDevice != nullptr &&
               GetModuleHandleEx ( GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT |
                                   GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
                           reinterpret_cast <LPCWSTR> (D3D12CreateDevice), &hModD3D12 ) != 0 )
          {
            ThrowIfFailed (
              D3D12CreateDevice (pAdapter, D3D_FEATURE_LEVEL_11_0,                                          IID_PPV_ARGS (&pD3D12Device.p )));
            ThrowIfFailed (
              pD3D12Device->CreateCommandQueue (
                                        std::array <D3D12_COMMAND_QUEUE_DESC, 1>
                                                { { D3D12_COMMAND_LIST_TYPE_DIRECT, 0,
                                                    D3D12_COMMAND_QUEUE_FLAG_NONE,  0 } }.data (),
                                                                                                            IID_PPV_ARGS (&pCmdQueue.p    )));
            ThrowIfFailed (
              pD3D12Device->CreateCommandAllocator (D3D12_COMMAND_LIST_TYPE_DIRECT,                         IID_PPV_ARGS (&pCmdAllocator.p)));
            ThrowIfFailed (
              pD3D12Device->CreateCommandList   (0, D3D12_COMMAND_LIST_TYPE_DIRECT, pCmdAllocator, nullptr, IID_PPV_ARGS (&pCmdList.p     )));

            DXGI_MODE_DESC bufferDesc { 2, 2, { 60, 1 },
              DXGI_FORMAT_R10G10B10A2_UNORM, DXGI_MODE_SCANLINE_ORDER_UNSPECIFIED,
                                             DXGI_MODE_SCALING_UNSPECIFIED };

            DXGI_SWAP_CHAIN_DESC swapDesc = {
                      bufferDesc, { 1, 0 }, DXGI_USAGE_RENDER_TARGET_OUTPUT,
                                    2,
            ...
            ThrowIfFailed (
              pFactory->CreateSwapChain (pCmdQueue, &swapDesc, &pSwapChain.p));

            static uintptr_t
                   *g_methodsTable =                    (uintptr_t  *)::calloc (      150,  sizeof (uintptr_t));
				  	memcpy (g_methodsTable,                    *(uintptr_t **)pD3D12Device.p,  44 * sizeof (uintptr_t));
				  	memcpy (g_methodsTable + 44,               *(uintptr_t **)pCmdQueue.p,     19 * sizeof (uintptr_t));
				  	memcpy (g_methodsTable + 44 + 19,          *(uintptr_t **)pCmdAllocator.p,  9 * sizeof (uintptr_t));
				  	memcpy (g_methodsTable + 44 + 19 + 9,      *(uintptr_t **)pCmdList.p,      60 * sizeof (uintptr_t));
				  	memcpy (g_methodsTable + 44 + 19 + 9 + 60, *(uintptr_t **)pSwapChain.p,    18 * sizeof (uintptr_t));

            _HookSwapChain ();

            SK_Win32_CleanupDummyWindow (swapDesc.OutputWindow);

            bHookedD3D12 = TRUE;
          }
        }

        catch (SK_ComException& /*_SK_ComE*/)
        {
          // D3D12 is unworkable
        }

        //auto D3D11CreateDeviceAndSwapChain = (D3D11CreateDeviceAndSwapChain_pfn) SK_GetProcAddress ( L"d3d11.dll",
        ...
        if ( config.apis.dxgi.d3d11.hook )//&&
             ...
             //GetModuleHandleEx ( GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT |
             //                    GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
             //            reinterpret_cast <LPCWSTR> (D3D11CreateDeviceAndSwapChain), &hModD3D11 ) != 0 )
        {
                D3D_FEATURE_LEVEL featureLevel;
	        const D3D_FEATURE_LEVEL featureLevels [] = { D3D_FEATURE_LEVEL_10_0, D3D_FEATURE_LEVEL_10_1,
                                                       D3D_FEATURE_LEVEL_11_0, D3D_FEATURE_LEVEL_11_1 };

          DXGI_MODE_DESC bufferDesc { 2, 2, { 60, 1 },
              DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_MODE_SCANLINE_ORDER_UNSPECIFIED,
                                          DXGI_MODE_SCALING_UNSPECIFIED };
          ...
          try
          {
            SK_ComPtr <ID3D11Device>        pD3D11Device;
            SK_ComPtr <ID3D11DeviceContext> pD3D11ImmediateContext;

            pAdapter.Release ();

            ThrowIfFailed (
              D3D11CreateDevice_Import (pAdapter, D3D_DRIVER_TYPE_HARDWARE, nullptr, 0x0, featureLevels, 4, D3D11_SDK_VERSION, &pD3D11Device.p, &featureLevel, &pD3D11ImmediateContext.p)
            );
              ...
              //                                                              &pSwapChain.p,
              //                                                            &pD3D11Device.p,
              //                                  &featureLevel,  &pD3D11ImmediateContext.p)
              //            );

            if (pSwapChain.p == nullptr)
              ThrowIfFailed (pFactory->CreateSwapChain (pD3D11Device, &swapDesc, &pSwapChain.p));
            ...
            static uintptr_t
                     *g_methodsTable =           (uintptr_t  *)::calloc (                205,  sizeof (uintptr_t));
					    memcpy (g_methodsTable,           *(uintptr_t **)pSwapChain.p,              18 * sizeof (uintptr_t));
					    memcpy (g_methodsTable + 18,      *(uintptr_t **)pD3D11Device.p,            43 * sizeof (uintptr_t));
					    memcpy (g_methodsTable + 18 + 43, *(uintptr_t **)pD3D11ImmediateContext.p, 144 * sizeof (uintptr_t));

            sk_hook_d3d11_t
                        d3d11_hook_ctx                    = { };
                        d3d11_hook_ctx.ppDevice           = &pD3D11Device.p;
                        d3d11_hook_ctx.ppImmediateContext = &pD3D11ImmediateContext.p;
            HookD3D11 (&d3d11_hook_ctx);
      ...
      catch (SK_ComException& /*_SK_ComE*/)
      {
        ...
        SK_ReleaseAssert (!"dxgi is b0rked");
      }
...
void ResetImGui_D3D12 (IDXGISwapChain* /*This*/)
{
  ...
  if (rb.api == SK_RenderAPI::D3D12)
  {
    void SK_CEGUI_QueueResetD3D12 (void);
             ...
             const DXGI_PRESENT_PARAMETERS *pPresentParameters )
{
  ...
  catch (const SK_SEH_IgnoredException&)
  {
    ...
    SK_D3D11_ResetTexCache   ();
    SK_CEGUI_QueueResetD3D11 (); // Prior to the next present, reset the UI
    SK_CEGUI_QueueResetD3D12 (); // Prior to the next present, reset the UI
    hr = S_OK;
  }
  SK_SEH_RemoveTranslator (orig_se);
...
)
{
  ...
  if (process/* || SK_GetFramesDrawn () < 2*/)
  {
    ...
    if ( FAILED (This->GetDevice (IID_ID3D11Device, (void **)&pDev.p)) )
    {
      // Late initialization
      //
      if (SUCCEEDED (This->GetDevice (IID_ID3D12Device, (void **)&pDev12.p)))
      {
        if (rb.api != SK_RenderAPI::D3D12)
        {
          if (! std::exchange (pLazyD3D12Chain, (IDXGISwapChain3 *)This))
          {
            SK_ComPtr <ID3D12CommandQueue>
                                 p12Queue;
            pLazyD3D12Device = pDev12;

            D3D12_COMMAND_QUEUE_DESC     queue_desc = { };
            pDev12->CreateCommandQueue (&queue_desc, IID_ID3D12CommandQueue, (void **)&p12Queue.p);
    ...
    if ( pDev != nullptr || rb.api == SK_RenderAPI::D3D12 )
    {
      ...
      auto _EndSwap = [&](void)
      {
        ...
        if (SUCCEEDED (hr))
        {
          ...
          else if (rb.api == SK_RenderAPI::D3D12)
            SK_D3D12_PostPresent ((ID3D12Device *)rb.device.p, This, hr);
        }
      };
                        ...
                        IDXGISwapChain **ppDest )
{
  ...
  if (pCmdQueue.p != nullptr)
  {
    ...
    *ppDest =
    ...
    SK_LOG0 ( (L" + SwapChain <IDXGISwapChain> (%08" PRIxPTR L"h) wrapped using D3D12 Command Queue",
             (uintptr_t)pSwapChain ),
           L"   DXGI   " );
    ...
    rb.swapchain = *ppDest;
    rb.device    = pDev.p;
    _d3d12_rbk->init ((IDXGISwapChain3 *)*ppDest, pCmdQueue.p);
...
//    return CreateSwapChainForHwnd_Original (This, pDevice, hWnd, pDesc, pFullscreenDesc, pRestrictToOutput, ppSwapChain);
  ...
  if ( SUCCEEDED (ret) )
  {
    ...
    {
      SK_D3D11_MakeCachedDeviceAndSwapChainForHwnd
           ( (IDXGISwapChain *)*ppSwapChain,  hWnd, (ID3D11Device *)pDevice );
    }                         (*ppSwapChain)->AddRef ();
