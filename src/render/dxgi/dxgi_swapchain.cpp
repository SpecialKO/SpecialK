...
IWrapDXGISwapChain::Present (UINT SyncInterval, UINT Flags)
{
  ...
  // ============================================================================
  // Vertex Shader: Passthrough with position + UV (568 bytes, Shader Model 4.0)
  static const BYTE g_vs_overlay_bytecode[] = {
    0x44, 0x58, 0x42, 0x43, 0x8f, 0x84, 0x5e, 0x2d, 0x6f, 0x6c, 0x28, 0x8f,
    0x85, 0xf1, 0xc8, 0x42, 0x3d, 0x90, 0x5f, 0x1a, 0x01, 0x00, 0x00, 0x00,
    0x34, 0x02, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x34, 0x00, 0x00, 0x00,
    0x8c, 0x00, 0x00, 0x00, 0xe0, 0x00, 0x00, 0x00, 0x5c, 0x01, 0x00, 0x00,
    0xb8, 0x01, 0x00, 0x00, 0x52, 0x44, 0x45, 0x46, 0x50, 0x00, 0x00, 0x00,
    ...
    0x53, 0x48, 0x44, 0x52, 0x54, 0x00, 0x00, 0x00, 0x40, 0x00, 0x01, 0x00,
    0x15, 0x00, 0x00, 0x00, 0x5f, 0x00, 0x00, 0x03, 0x32, 0x10, 0x10, 0x00,
  ...
  // Pixel Shader: Sample texture and output (576 bytes, Shader Model 4.0)
  static const BYTE g_ps_overlay_bytecode[] = {
    ,
    ...
    0x00, 0x00, 0x00, 0x00, 0x45, 0x00, 0x00, 0x08, 0xf2, 0x20, 0x10, 0x00,
  ...
  // Shader resources (created once, reused each frame)
  static ID3D11VertexShader*       s_overlay_vs = nullptr;
  static ID3D11PixelShader*        s_overlay_ps = nullptr;
  static ID3D11InputLayout*        s_overlay_layout = nullptr;
  static ID3D11Buffer*             s_overlay_vb = nullptr;
  static ID3D11SamplerState*       s_overlay_sampler = nullptr;
  static ID3D11ShaderResourceView* s_overlay_srv = nullptr;

  // SKF1 Runtime Status Struct - Single source of truth for entire pipeline state
      ...
      if (SUCCEEDED (hr) && bb != nullptr && ctx != nullptr)
      {
            ...
            bbDesc.Format == DXGI_FORMAT_R10G10B10A2_UNORM)
        {
          ...
          if (s_skf1.has_frame && s_skf1.tex != nullptr)
          {
            ...
            if (!formats_match)
            {
              // FORMAT MISMATCH: Use shader-based blit for format conversion (BGRA8 → R10G10B10A2)
              static std::atomic<bool> s_logged_shader_path = false;
              if (!s_logged_shader_path.exchange(true))
              {
                _SidecarLog(L"→ FORMAT MISMATCH: overlay tex=%u backbuffer=%u", s_skf1.texFmt, bbDesc.Format);
                _SidecarLog(L"→ Using SHADER path for format conversion (BGRA8 → R10G10B10A2)");
              }
              
              // Create shader resources (one-time initialization)
              if (s_overlay_vs == nullptr)
              {
                HRESULT hr_vs = dev->CreateVertexShader(g_vs_overlay_bytecode, sizeof(g_vs_overlay_bytecode), nullptr, &s_overlay_vs);
                HRESULT hr_ps = dev->CreatePixelShader(g_ps_overlay_bytecode, sizeof(g_ps_overlay_bytecode), nullptr, &s_overlay_ps);
                ...
                HRESULT hr_layout = dev->CreateInputLayout(layout, 2, g_vs_overlay_bytecode, sizeof(g_vs_overlay_bytecode), &s_overlay_layout);
                
                // Vertex buffer (full-screen quad in NDC: -1 to 1)
                ...
                HRESULT hr_vb = dev->CreateBuffer(&vbDesc, &vbData, &s_overlay_vb);
                ...
                HRESULT hr_samp = dev->CreateSamplerState(&sampDesc, &s_overlay_sampler);
                
                // Shader Resource View for overlay texture
                D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
                srvDesc.Format = DXGI_FORMAT_B8G8R8A8_UNORM;
                srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
                srvDesc.Texture2D.MostDetailedMip = 0;
                srvDesc.Texture2D.MipLevels = 1;
                HRESULT hr_srv = dev->CreateShaderResourceView(s_skf1.tex, &srvDesc, &s_overlay_srv);
                
                if (FAILED(hr_vs) || FAILED(hr_ps) || FAILED(hr_layout) || FAILED(hr_vb) || FAILED(hr_samp) || FAILED(hr_srv))
                {
                  _SidecarLog(L"→ Shader resource creation FAILED: vs=%08X ps=%08X layout=%08X vb=%08X samp=%08X srv=%08X",
                              hr_vs, hr_ps, hr_layout, hr_vb, hr_samp, hr_srv);
                  if (s_overlay_vs) { s_overlay_vs->Release(); s_overlay_vs = nullptr; }
                  if (s_overlay_ps) { s_overlay_ps->Release(); s_overlay_ps = nullptr; }
                  if (s_overlay_layout) { s_overlay_layout->Release(); s_overlay_layout = nullptr; }
                  if (s_overlay_vb) { s_overlay_vb->Release(); s_overlay_vb = nullptr; }
                  if (s_overlay_sampler) { s_overlay_sampler->Release(); s_overlay_sampler = nullptr; }
                  if (s_overlay_srv) { s_overlay_srv->Release(); s_overlay_srv = nullptr; }
                  dev->Release();
                  return SK_DXGI_DispatchPresent (pReal, SyncInterval, Flags, nullptr, SK_DXGI_PresentSource::Wrapper);
                }
                
                _SidecarLog(L"→ Shader resources created successfully");
              }
              
              // Per-frame rendering: Save state, draw, restore state
              if (s_overlay_vs != nullptr && s_overlay_srv != nullptr)
              {
                // Save current D3D11 state
                D3D11_VIEWPORT saved_vp;
                UINT num_vps = 1;
                ctx->RSGetViewports(&num_vps, &saved_vp);
                
                D3D11_RECT saved_scissor;
                UINT num_scissors = 1;
                ctx->RSGetScissorRects(&num_scissors, &saved_scissor);
                ...
                ID3D11ShaderResourceView* saved_ps_srv = nullptr;
                ctx->PSGetShaderResources(0, 1, &saved_ps_srv);
                
                ID3D11SamplerState* saved_ps_samp = nullptr;
                ctx->PSGetSamplers(0, 1, &saved_ps_samp);
                ...
                rtvDesc.Format = bbDesc.Format;  // R10G10B10A2
                rtvDesc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
                rtvDesc.Texture2D.MipSlice = 0;
                ID3D11RenderTargetView* bb_rtv = nullptr;
                HRESULT hr_rtv = dev->CreateRenderTargetView(bb, &rtvDesc, &bb_rtv);
                
                if (SUCCEEDED(hr_rtv) && bb_rtv != nullptr)
                {
                  // Set viewport and scissor for 256×256 at (0,0)
                  D3D11_VIEWPORT vp = { 0.0f, 0.0f, 256.0f, 256.0f, 0.0f, 1.0f };
                  ctx->RSSetViewports(1, &vp);
                  
                  D3D11_RECT scissor = { 0, 0, 256, 256 };
                  ctx->RSSetScissorRects(1, &scissor);
                  
                  // Bind shaders and resources
                  ctx->VSSetShader(s_overlay_vs, nullptr, 0);
                  ctx->PSSetShader(s_overlay_ps, nullptr, 0);
                  ctx->IASetInputLayout(s_overlay_layout);
                  
                  UINT stride = 16;  // sizeof(Vertex)
                  UINT offset = 0;
                  ctx->IASetVertexBuffers(0, 1, &s_overlay_vb, &stride, &offset);
                  ctx->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
                  
                  ctx->PSSetShaderResources(0, 1, &s_overlay_srv);
                  ctx->PSSetSamplers(0, 1, &s_overlay_sampler);
                  
                  ctx->OMSetRenderTargets(1, &bb_rtv, nullptr);
                  
                  // Draw the quad (D3D11 automatically converts BGRA8 → R10G10B10A2)
                  ctx->Draw(4, 0);
                  
                  // Restore state
                  ctx->RSSetViewports(1, &saved_vp);
                  ctx->RSSetScissorRects(1, &saved_scissor);
                  ctx->OMSetRenderTargets(1, &saved_rtv, saved_dsv);
                  ctx->PSSetShaderResources(0, 1, &saved_ps_srv);
                  ctx->PSSetSamplers(0, 1, &saved_ps_samp);
                  ...
                  {
                    _SidecarLog(L"SKF1 Stage F OK: Blit executed (shader path)");
                  }
                ...
                {
                  _SidecarLog(L"→ CreateRenderTargetView FAILED: hr=%08X", hr_rtv);
                }
                
                // Release saved state objects
                if (saved_rtv) saved_rtv->Release();
                if (saved_dsv) saved_dsv->Release();
                if (saved_ps_srv) saved_ps_srv->Release();
                if (saved_ps_samp) saved_ps_samp->Release();
                if (saved_vs) saved_vs->Release();
                if (saved_ps) saved_ps->Release();
                if (saved_layout) saved_layout->Release();
                if (saved_vb) saved_vb->Release();
              }
            }
            else
            {
              // Formats match - safe to use CopySubresourceRegion (fast path)
