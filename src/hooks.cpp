                    ...
                    LPVOID *ppOriginal )
{
  ...
  {
    return SH_Introspect (pTarget, SH_TRAMPOLINE, ppOriginal);
  }
                      ...
                      UINT    idx )
{
  ...
  {
    return SK_CreateFuncHookEx (pwszFuncName, pTarget, pDetour, ppOriginal, idx+1);
  }
  ...
  if (status != MH_OK)
  {
    if (status == MH_ERROR_ALREADY_CREATED)
    {
      if (ppOriginal == nullptr) {
        ...
        status = SK_CreateFuncHookEx (pwszModule, pFuncAddr, pDetour, ppOriginal, 1);
    }
  }
