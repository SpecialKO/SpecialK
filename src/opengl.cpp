...
//
// Most of the GL context state is thread-local, but we do
//   maintain a map of shared contexts that will necessitate
//     locking.
//
static SK_Thread_HybridSpinlock *cs_gl_ctx = nullptr;
...
BOOL
CallSwap (HDC hDC, LPVOID pfnSwapFunc)
{
  __try {
    return
      static_cast <wglSwapBuffers_pfn> (pfnSwapFunc)(hDC);
...
SK_GL_SwapBuffers (HDC hDC, LPVOID pfnSwapFunc)
{
  ...
  if (                  __gl_current_hglrc &&
       (! init_.count (__gl_current_hglrc) || need_init) ||
       (! __gl_primary_context) )
  {
    if (               __gl_primary_context == 0 &&
         WindowFromDC (__gl_current_hdc)    == WindowFromDC (hDC) )
    {
      if (__gl_shared_contexts.count (__gl_current_hglrc))
      {
        if (__gl_shared_contexts.count (__gl_shared_contexts [__gl_current_hglrc]))
          __gl_primary_context = __gl_shared_contexts [__gl_shared_contexts [__gl_current_hglrc]];
        else
          __gl_primary_context = __gl_shared_contexts [__gl_current_hglrc];
      }
      else
        __gl_primary_context = __gl_current_hglrc;
    }

    if (__gl_primary_context && (! init_ [__gl_primary_context]))
    {
      glewExperimental = GL_TRUE;
      glewInit ();

      ImGui_ImplGL3_Init ();
      init_ [__gl_primary_context] = TRUE;
    }
...
//HWND  hWnd = __gl_current_hwnd;
  ...
  if ( init_ [__gl_primary_context] &&
               (   __gl_primary_context ==     __gl_current_hglrc   ||
                 ( __gl_shared_contexts.count (__gl_current_hglrc)  &&
                                               __gl_primary_context == __gl_shared_contexts [__gl_current_hglrc] ) ||
                   __gl_shared_contexts.count (__gl_current_hglrc)  &&
                 ( __gl_shared_contexts.count (__gl_shared_contexts [__gl_current_hglrc]) &&
                                              __gl_primary_context == __gl_shared_contexts [__gl_shared_contexts [__gl_current_hglrc]] )
               ) &&
      WindowFromDC (__gl_current_hdc)   == WindowFromDC (hDC) )
  {
...
SK_GL_TrackHDC (HDC hDC)
{
  if (WindowFromDC (__gl_current_hdc) == WindowFromDC (hDC) && WindowFromDC (hDC) == GetActiveWindow ())
  {
    // Setup our window message hook for the command console
...
wglSwapBuffers (HDC hDC)
{
  ...
  InterlockedCompareExchangePointer ( (void **)&wgl_swap_buffers, GetProcAddress (local_gl, "wglSwapBuffers"), nullptr );
...
SwapBuffers (HDC hDC)
{
  ...
  InterlockedCompareExchangePointer ( (void **)&gdi_swap_buffers, GetProcAddress (GetModuleHandle (L"gdi32full.dll"), "SwapBuffers"), nullptr );
...
SK_HookGL (void)
{
  ...
  if (! InterlockedCompareExchange (&SK_GL_initialized, TRUE, FALSE))
  {
    ...
    LoadLibraryW_Original (L"gdi32full.dll");
...
wglShareLists (HGLRC ctx0, HGLRC ctx1)
{
  ...
  if (ret == TRUE)
  {
    InterlockedCompareExchangePointer ((void **)&__gl_primary_context, ctx0, nullptr);

    std::lock_guard <SK_Thread_HybridSpinlock> auto_lock (*cs_gl_ctx);

    // Multiple sharing is unexpected
    assert ( __gl_shared_contexts.count (ctx1) == 0 ||
                   __gl_shared_contexts [ctx1] == ctx0 );
