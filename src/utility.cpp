...
SK_GetBlacklistFilename (void)
{
  ...
  if (! InterlockedCompareExchange (&init, TRUE, FALSE))
  {
    *host_proc.wszBlacklist = L'\0';

    lstrcatW             (host_proc.wszBlacklist, SK_GetHostPath ());
    PathAppendW          (host_proc.wszBlacklist, L"SpecialK.deny.");
    lstrcatW             (host_proc.wszBlacklist, SK_GetHostApp  ());
    PathRemoveExtensionW (host_proc.wszBlacklist);
  }
...
SK_GetHostPath (void)
{
  ...
  if (! InterlockedCompareExchange (&init, TRUE, FALSE))
  {
    ...
    PathRemoveBackslashW (wszProcessName);
...
SK_DeleteTemporaryFiles (const wchar_t* wszPath, const wchar_t* wszPattern)
{
  ...
  lstrcatW            (wszFindPattern, wszPath);
  PathRemoveBackslash (wszFindPattern);
  PathAppendW         (wszFindPattern, wszPattern);
    ...
    do
    {
      if (fd.dwFileAttributes != INVALID_FILE_ATTRIBUTES)
      {
        *wszFullPath = L'\0';
        ...
        lstrcatW    (wszFullPath, wszPath);
        PathAppendW (wszFullPath, fd.cFileName);
