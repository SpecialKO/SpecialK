...
bool ImGui::ButtonEx (const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
  ...
  // Render
  const ImColor col = GetStyleColorVec4 ((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
...
bool ImGui::CloseButton (ImGuiID id, const ImVec2& pos, float radius)
{
  ...
  if (hovered)
    window->DrawList->AddCircleFilled (center, ImMax (2.0f, radius), GetStyleColorVec4 (held ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered), 9);

  float cross_extent = (radius * 0.7071f) - 1.0f;
  ImColor cross_col = GetStyleColorVec4 (ImGuiCol_Text);
...
bool ImGui::CollapseButton (ImGuiID id, const ImVec2& pos)
{
  ...
  ImColor col = GetStyleColorVec4 ((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
  if (hovered || held)
...
void ImGui::Scrollbar (ImGuiAxis axis)
{
  ...
  window->DrawList->AddRectFilled (bb.Min, bb.Max, GetStyleColorVec4 (ImGuiCol_ScrollbarBg), window->WindowRounding, window_rounding_corners);
  ...
  // Render grab
  const ImColor grab_col = GetStyleColorVec4 (held ? ImGuiCol_ScrollbarGrabActive : hovered ? ImGuiCol_ScrollbarGrabHovered : ImGuiCol_ScrollbarGrab, alpha);
  ImRect grab_rect;
  if (horizontal)
...
bool ImGui::ImageButton (ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
  ...
  window->DrawList->AddImage (user_texture_id, image_bb.Min, image_bb.Max, uv0, uv1, tint_col);
...
bool ImGui::Checkbox (const char* label, bool* v)
{
  ...
  RenderFrame (check_bb.Min, check_bb.Max, GetStyleColorVec4 ((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
  if (*v)
  {
    const float pad = ImMax (1.0f, (float)(int)(square_sz / 6.0f));
    RenderCheckMark (check_bb.Min + ImVec2 (pad, pad), GetStyleColorVec4 (ImGuiCol_CheckMark), square_sz - pad * 2.0f);
  }
...
bool ImGui::RadioButton (const char* label, bool active)
{
  ...
  RenderNavHighlight (total_bb, id);
  window->DrawList->AddCircleFilled (center, radius, GetStyleColorVec4 ((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 16);
  if (active)
  {
    const float pad = ImMax (1.0f, (float)(int)(square_sz / 6.0f));
    window->DrawList->AddCircleFilled (center, radius - pad, GetStyleColorVec4 (ImGuiCol_CheckMark), 16);
  }

  if (style.FrameBorderSize > 0.0f)
  {
    window->DrawList->AddCircle (center + ImVec2 (1, 1), radius, GetStyleColorVec4 (ImGuiCol_BorderShadow), 16, style.FrameBorderSize);
    window->DrawList->AddCircle (center, radius, GetStyleColorVec4 (ImGuiCol_Border), 16, style.FrameBorderSize);
  }
...
void ImGui::ProgressBar (float fraction, const ImVec2& size_arg, const char* overlay)
{
  ...
  RenderFrame (bb.Min, bb.Max, GetStyleColorVec4 (ImGuiCol_FrameBg), true, style.FrameRounding);
  bb.Expand (ImVec2 (-style.FrameBorderSize, -style.FrameBorderSize));
  const ImVec2 fill_br = ImVec2 (ImLerp (bb.Min.x, bb.Max.x, fraction), bb.Max.y);
  RenderRectFilledRangeH (window->DrawList, bb, GetStyleColorVec4 (ImGuiCol_PlotHistogram), 0.0f, fraction, style.FrameRounding);
...
void ImGui::Separator ()
{
  ...
  window->DrawList->AddLine (bb.Min, ImVec2 (bb.Max.x, bb.Min.y), GetStyleColorVec4 (ImGuiCol_Separator));
...
void ImGui::VerticalSeparator ()
{
  ...
  window->DrawList->AddLine (ImVec2 (bb.Min.x, bb.Min.y), ImVec2 (bb.Min.x, bb.Max.y), GetStyleColorVec4 (ImGuiCol_Separator));
  if (g.LogEnabled)
    LogText (" |");
}
...
bool ImGui::SplitterBehavior (const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay)
{
  ...
  // Render
  const ImColor col = GetStyleColorVec4 (held ? ImGuiCol_SeparatorActive : (hovered && g.HoveredIdTimer >= hover_visibility_delay) ? ImGuiCol_SeparatorHovered : ImGuiCol_Separator);
...
bool ImGui::BeginCombo (const char* label, const char* preview_value, ImGuiComboFlags flags)
{
  ...
  const ImColor frame_col = GetStyleColorVec4 (hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
  RenderNavHighlight (frame_bb, id);
  if (!(flags & ImGuiComboFlags_NoPreview))
    window->DrawList->AddRectFilled (frame_bb.Min, ImVec2 (frame_bb.Max.x - arrow_size, frame_bb.Max.y), frame_col, style.FrameRounding, ImDrawCornerFlags_Left);
  if (!(flags & ImGuiComboFlags_NoArrowButton))
  {
    window->DrawList->AddRectFilled (ImVec2 (frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetStyleColorVec4 ((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button), style.FrameRounding, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
...
bool ImGui::DragScalar (const char* label, ImGuiDataType data_type, void* v, float v_speed, const void* v_min, const void* v_max, const char* format, float power)
{
  ...
  // Draw frame
  const ImColor frame_col = GetStyleColorVec4 (g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
  RenderNavHighlight (frame_bb, id);
...
bool ImGui::SliderScalar (const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
  ...
  // Render grab
  window->DrawList->AddRectFilled (grab_bb.Min, grab_bb.Max, GetStyleColorVec4 (g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::InputTextEx (const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
  ...
  {
    RenderNavHighlight (frame_bb, id);
    RenderFrame (frame_bb.Min, frame_bb.Max, GetStyleColorVec4 (ImGuiCol_FrameBg), true, style.FrameRounding);
  }
  ...
  if (render_cursor || render_selection)
  {
    ...
    if (render_selection)
    {
      ...
      ImColor bg_color   = GetStyleColorVec4 (ImGuiCol_TextSelectedBg, render_cursor ? 1.0f : 0.6f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
    ...
    {
      ImColor col = GetStyleColorVec4 (is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
    ...
    if (render_cursor)
    {
      ...
      if (cursor_is_visible && cursor_screen_rect.Overlaps (clip_rect))
        draw_window->DrawList->AddLine (cursor_screen_rect.Min, cursor_screen_rect.GetBL (), GetStyleColorVec4 (ImGuiCol_Text));
...
static inline ImColor ImAlphaBlendColor (ImU32 col_a, ImU32 col_b)
{
  ...
  return ImColor ((float)r/255.0f, (float)g/255.0f, (float)b/255.0f, 1.0f);
...
void ImGui::RenderColorRectWithAlphaCheckerboard (ImVec2 p_min, ImVec2 p_max, const ImColor& col, float grid_step, ImVec2 grid_off, float rounding, int rounding_corners_flags)
{
  ImGuiWindow* window = GetCurrentWindow ();
  if ((((unsigned int)col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF)
  {
    ImColor col_bg1 = ImAlphaBlendColor (IM_COL32 (204, 204, 204, 255), col);
    ImColor col_bg2 = ImAlphaBlendColor (IM_COL32 (128, 128, 128, 255), col);
...
bool ImGui::ColorButton (const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
  ...
  ImColor col_rgb_without_alpha (col_rgb.x, col_rgb.y, col_rgb.z, 1.0f);
  ...
  if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
  {
    ...
    RenderColorRectWithAlphaCheckerboard (ImVec2 (bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, col_rgb, grid_step, ImVec2 (-grid_step + off, off), rounding, ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotRight);
    window->DrawList->AddRectFilled (bb_inner.Min, ImVec2 (mid_x, bb_inner.Max.y), col_rgb_without_alpha, rounding, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft);
  }
  else
  {
    ...
    if (col_source.Value.w < 1.0f)
      ...
      window->DrawList->AddRectFilled (bb_inner.Min, bb_inner.Max, col_source, rounding, ImDrawCornerFlags_All);
  }
  RenderNavHighlight (bb, id);
  if (g.Style.FrameBorderSize > 0.0f)
    RenderFrameBorder (bb.Min, bb.Max, rounding);
  else
    window->DrawList->AddRect (bb.Min, bb.Max, GetStyleColorVec4 (ImGuiCol_FrameBg), rounding); // Color button are often in need of some sort of border
...
bool ImGui::TreeNodeBehavior (ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
  ...
  // Render
  const ImColor col      = GetStyleColorVec4 ((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
...
bool
ImGui::ListBoxHeader ( const char*   label,
                       const ImVec2& size_arg )
{
  ...
  ImVec2 frame_size =
    ImVec2 ( size.x, ImMax ( size.y, label_size.y ) );
...
void
ImGui::PlotLines ( const char*  label,        const float* values,
                         int    values_count,       int    values_offset,
                   const char*  overlay_text,       float  scale_min,
                                                    float  scale_max,
                         ImVec2 graph_size,         int    stride )
{
  ...
  PlotEx (
    ImGuiPlotType_Lines, label,
      &Plot_ArrayGetter, (void *)&data,
...
bool ImGui::MenuItem (const char* label, const char* shortcut, bool selected, bool enabled)
{
  ...
  else
  {
    ...
    if (selected)
      RenderCheckMark (pos + ImVec2 (window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.40f, g.FontSize * 0.134f * 0.5f), GetStyleColorVec4 (enabled ? ImGuiCol_Text : ImGuiCol_TextDisabled), g.FontSize * 0.866f);
  }
...
namespace ImGui
{
  ...
  static float         TabBarCalcMaxTabWidth    (void);
...
static int
IMGUI_CDECL
TabBarSortItemComparer (const void* lhs, const void* rhs)
{
...
static ImGuiTabBar*
GetTabBarFromTabBarRef (const ImGuiTabBarRef& ref)
{
  ImGuiContext& g = *GImGui;

  return ref.Ptr ?
         ref.Ptr : g.TabBars.GetByIndex (ref.IndexInMainPool);
}
...
ImGui::BeginTabBar (const char* str_id, ImGuiTabBarFlags flags)
{
  ...
  return
    BeginTabBarEx (tab_bar, tab_bar_bb, flags | ImGuiTabBarFlags_IsFocused);
}
...
ImGui::BeginTabBarEx (ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
  ...
  // Draw separator
  const ImColor col = GetStyleColorVec4 ((flags & ImGuiTabBarFlags_IsFocused) ? ImGuiCol_TabActive : ImGuiCol_Tab);
...
ImGui::EndTabBar (void)
{
  ...
  g.CurrentTabBar =
    g.CurrentTabBarStack.empty () ? NULL :
...
static void ImGui::TabBarLayout (ImGuiTabBar* tab_bar)
{
  ...
  for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
  {
    ...
    width_total_contents +=
      ( tab_n > 0 ? g.Style.ItemInnerSpacing.x
                  : 0.0f ) + tab->WidthContents;
  ...
  const float width_avail  = tab_bar->BarRect.GetWidth ();
        float width_excess =
          ( width_avail < width_total_contents ) ?
...
bool    ImGui::TabItemEx (ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
  ...
  const ImColor tab_col = GetStyleColorVec4 ((held || hovered) ? ImGuiCol_TabHovered : tab_contents_visible ? (tab_bar_focused ? ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive) : (tab_bar_focused ? ImGuiCol_Tab : ImGuiCol_TabUnfocused));
...
void
ImGui::TabItemBackground (ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, const ImColor& col)
{
  ...
  if (g.Style.TabBorderSize > 0.0f)
  {
    ...
    draw_list->PathStroke (GetStyleColorVec4 (ImGuiCol_Border), false, g.Style.TabBorderSize);
  }
}
...
bool ImGui::TabItemLabelAndCloseButton (ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id)
{
  ...
  if (label_size.x > text_ellipsis_clip_bb.GetWidth ())
  {
    ...
    if (!close_button_visible && ellipsis_x + ellipsis_width <= bb.Max.x)
      RenderPixelEllipsis (draw_list, ImVec2 (ellipsis_x, text_pixel_clip_bb.Min.y), ellipsis_dot_count, GetStyleColorVec4 (ImGuiCol_Text));
  }
  else
  {
