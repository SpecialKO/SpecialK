...
static void*   MallocWrapper(size_t size, void* user_data)    { IM_UNUSED(user_data); return _aligned_malloc(size,16); }
static void    FreeWrapper(void* ptr, void* user_data)        { IM_UNUSED(user_data); _aligned_free(ptr); }
#else
...
const ImVec4& ImGui::GetStyleColorVec4 (ImGuiCol idx, float alpha_mul)
{
  ImGuiStyle* pStyle =
    &GImGui->Style;
  ...
  if (idx >= ImGuiCol_COUNT || idx < 0)
      idx  = ImGuiCol_Text;

  if (alpha_mul != 1.0f)
  {
    color =
      sk_gsl::at <ImVec4, ImGuiCol_COUNT> (pStyle->Colors, idx)
                * ImVec4 (1.f, 1.f, 1.f, alpha_mul);
  }
...
const ImColor& ImGui::GetStyleAlphaModulatedColor (const ImColor& col, ImColor& modulated)
{
  ImGuiStyle* pStyle =
    &GImGui->Style;
...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
    ...
    {
        window->DrawList->AddText(g.Font, g.FontSize, pos, GetStyleColorVec4 (ImGuiCol_Text), text, text_display_end);
        if (g.LogEnabled)
...
void ImGui::RenderTextWrapped(ImVec2 pos, const char* text, const char* text_end, float wrap_width)
{
    ...
    if (text != text_end)
    {
        window->DrawList->AddText(g.Font, g.FontSize, pos, GetStyleColorVec4 (ImGuiCol_Text), text, text_end, wrap_width);
        if (g.LogEnabled)
...
void ImGui::RenderTextClippedEx(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    ...
    if (need_clipping)
    {
        ...
        draw_list->AddText(NULL, 0.0f, pos, GetStyleColorVec4(ImGuiCol_Text), text, text_display_end, 0.0f, &fine_clip_rect);
    }
    else
    {
        draw_list->AddText (NULL, 0.0f, pos, GetStyleColorVec4(ImGuiCol_Text), text, text_display_end, 0.0f, NULL);
    }
}
...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, const ImColor& fill_col, bool border, float rounding)
{
    ...
    {
        window->DrawList->AddRect ( p_min + ImVec2 (1.0f, 1.0f),
                                    p_max + ImVec2 (1.0f, 1.0f),
                                      GetStyleColorVec4 (ImGuiCol_BorderShadow),
...
void ImGui::RenderFrameBorder(ImVec2 p_min, ImVec2 p_max, float rounding)
{
    ...
    if (border_size > 0.0f)
    {
        window->DrawList->AddRect ( p_min + ImVec2 (1.0f, 1.0f),
                                    p_max + ImVec2 (1.0f, 1.0f),
                                      GetStyleColorVec4 (ImGuiCol_BorderShadow), rounding,
                                                         ImDrawCornerFlags_All,  border_size );
        window->DrawList->AddRect ( p_min, p_max,
                                      GetStyleColorVec4 (ImGuiCol_Border), rounding,
                                                         ImDrawCornerFlags_All, border_size );
    }
}
...
void ImGui::RenderArrow (ImVec2 p_min, ImGuiDir dir, float scale)
{
    ...
    const float  h      = g.FontSize * 1.00f;
          ...
          ImVec2 center = p_min + ImVec2 ( h * 0.50f,
                                           h * 0.50f * scale );
...
void ImGui::RenderBullet(ImVec2 pos)
{
    ...
    window->DrawList->AddCircleFilled (
      pos, g.FontSize * 0.20f,
        GetStyleColorVec4 (ImGuiCol_Text), 8
    );
}

void ImGui::RenderCheckMark (ImVec2 pos, const ImColor& col, float sz)
{
    ImGuiContext& g      = *GImGui;
    ImGuiWindow*  window = g.CurrentWindow;
...
void ImGui::RenderNavHighlight (const ImRect& bb, ImGuiID id, ImGuiNavHighlightFlags flags)
{
    ...
    {
        window->DrawList->AddRect(display_rect.Min, display_rect.Max, GetStyleColorVec4(ImGuiCol_NavHighlight), rounding, ~0, 1.0f);
    }
}
...
static void ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImColor resize_grip_col[4])
{
    ImGuiContext& g = *GImGui;
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
    {
        ...
        if (resize_grip_n == 0 || held || hovered)
            resize_grip_col[resize_grip_n] = GetStyleColorVec4(held ? ImGuiCol_ResizeGripActive : hovered ? ImGuiCol_ResizeGripHovered : ImGuiCol_ResizeGrip);
    }
    ...
    if (g.NavWindowingTarget && g.NavWindowingTarget->RootWindow == window)
    {
        ...
        if (nav_resize_delta.x != 0.0f || nav_resize_delta.y != 0.0f)
        {
            ...
            g.NavDisableMouseHover = true;
            resize_grip_col[0] = GetStyleColorVec4 (ImGuiCol_ResizeGripActive);
...
static void ImGui::RenderOuterBorders(ImGuiWindow* window)
{
    ...
    if (border_size > 0.0f && !(window->Flags & ImGuiWindowFlags_NoBackground))
        window->DrawList->AddRect(window->Pos, window->Pos + window->Size, GetStyleColorVec4(ImGuiCol_Border), rounding, ImDrawCornerFlags_All, border_size);
    ...
    if (border_held != -1)
    {
        ...
        window->DrawList->PathStroke(GetStyleColorVec4(ImGuiCol_SeparatorActive), false, ImMax(2.0f, border_size)); // Thicker than usual
    }
    if (g.Style.FrameBorderSize > 0 && !(window->Flags & ImGuiWindowFlags_NoTitleBar))
    {
        float y = window->Pos.y + window->TitleBarHeight() - 1;
        window->DrawList->AddLine(ImVec2(window->Pos.x + border_size, y), ImVec2(window->Pos.x + window->Size.x - border_size, y), GetStyleColorVec4(ImGuiCol_Border), g.Style.FrameBorderSize);
    }
}
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        {
          const ImVec4 dim_bg_col = ImVec4 (GetStyleColorVec4 (dim_bg_for_modal ? ImGuiCol_ModalWindowDimBg : ImGuiCol_NavWindowingDimBg)) *
                                    ImVec4 (1.f, 1.f, 1.f, g.DimBgRatio);
        ...
        if (dim_bg_for_window_list && window == g.NavWindowingTargetAnim)
        {
            ...
            if (!bb.Contains(viewport_rect)) // Avoid drawing if the window covers all the viewport anyway
                window->DrawList->AddRectFilled(bb.Min, bb.Max, GetStyleColorVec4 (ImGuiCol_NavWindowingHighlight, g.NavWindowingHighlightAlpha * 0.25f), g.Style.WindowRounding);
        }
        ...
        if (window->Collapsed)
        {
            ...
            ImColor title_bar_col = GetStyleColorVec4((title_bar_is_highlight && !g.NavDisableHighlight) ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBgCollapsed);
        ...
        else
        {
            ...
            {
                ImColor bg_col = GetStyleColorVec4 (GetWindowBgColorIdxFromFlags(flags));
                float alpha = 1.0f;
                if (g.NextWindowData.BgAlphaCond != 0)
                    alpha = g.NextWindowData.BgAlphaVal;
                if (alpha != 1.0f)
                    bg_col = ImColor (((unsigned int)bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha) << IM_COL32_A_SHIFT));
            ...
            {
                ImColor title_bar_col = GetStyleColorVec4(title_bar_is_highlight ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg);
            ...
            if (flags & ImGuiWindowFlags_MenuBar)
            {
                ...
                window->DrawList->AddRectFilled(menu_bar_rect.Min, menu_bar_rect.Max, GetStyleColorVec4 (ImGuiCol_MenuBarBg), (flags & ImGuiWindowFlags_NoTitleBar) ? window_rounding : 0.0f, ImDrawCornerFlags_Top);
                if (style.FrameBorderSize > 0.0f && menu_bar_rect.Max.y < window->Pos.y + window->Size.y)
                    window->DrawList->AddLine(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), GetStyleColorVec4 (ImGuiCol_Border), style.FrameBorderSize);
            }
        ...
        if (g.NavWindowingTargetAnim == window)
        {
            ...
            window->DrawList->AddRect(bb.Min, bb.Max, GetStyleColorVec4 (ImGuiCol_NavWindowingHighlight) + ImVec4 (0.f, 0.f, 0.f, g.NavWindowingHighlightAlpha), rounding, ~0, 3.0f);
        }
        ...
        if ((flags & ImGuiWindowFlags_ChildWindow) && (parent_window != nullptr && window->DC.ItemFlags != parent_window->DC.ItemFlags))
        {
...
{
  PushStyleColor (idx, ImColor ( col.x, col.y,
                                 col.z, col.w ));
}
...
bool ImGui::IsWindowHovered(ImGuiHoveredFlags flags)
{
  if (GImGui == nullptr)
    ...
    // Special K Mod:  Check HoveredWindow for null before testing content hoverable
    //
    if (g.HoveredWindow == NULL || !IsWindowContentHoverable(g.HoveredWindow, flags))
        return false;
    if (!(flags & ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
...
static void ImGui::NavUpdate()
{
    ...
    if (nav_keyboard_active)
    {
        ...
        #define NAV_MAP_LETTER(_LETTER,_NAV_INPUT) if (IsKeyDown (g.IO.KeysDown [(_LETTER)])) { g.IO.NavInputs [(_NAV_INPUT)] = 1.0f; g.NavInputSource = ImGuiInputSource_NavKeyboard; }
        ...
        if (! g.IO.KeyCtrl)
        {
          NAV_MAP_LETTER (SK_KeyMap_LeftHand_Arrow ('A'),
                                          ImGuiNavInput_KeyLeft_ );
          NAV_MAP_LETTER (SK_KeyMap_LeftHand_Arrow ('D'),
                                          ImGuiNavInput_KeyRight_);
          NAV_MAP_LETTER (SK_KeyMap_LeftHand_Arrow ('W'),
                                          ImGuiNavInput_KeyUp_   );
          NAV_MAP_LETTER (SK_KeyMap_LeftHand_Arrow ('S'),
...
void ImGui::EndColumns()
{
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            const ImVec4 col = GetStyleColorVec4 (held ? ImGuiCol_SeparatorActive : hovered ? ImGuiCol_SeparatorHovered : ImGuiCol_Separator);
...
const ImGuiPayload* ImGui::AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_AcceptNoDrawDefaultRect) && payload.Preview)
    {
        ...
        window->DrawList->AddRect(r.Min, r.Max, GetStyleColorVec4 (ImGuiCol_DragDropTarget), 0.0f, ~0, 2.0f);
