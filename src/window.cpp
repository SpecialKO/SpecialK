...
SK_ExpandSmallClipCursor (RECT *lpRect)
{
      ...
      0.75f * ( window.bottom -  window.top)  )
  {
    ...
    static constexpr DWORD dwReportingInterval = 30000UL;

    // Throttle this message
    if (dwLastReported < dwNow - dwReportingInterval)
    {
...
SK_Window_RepositionIfNeeded (void)
{
  ...
  SK_RunOnce (
      ...
      DWORD
    {
      ...
      while ( dwWaitState == _WorkSignal )
      {
        dwWaitState =
          WaitForMultipleObjects (2, hSignals, FALSE, INFINITE);

        if (dwWaitState == _WorkSignal)
                      ...
                      _In_  LPARAM lParam )
{
  ...
  {
    // Doubtful, but maybe someday (... please?!)
#ifdef MICROSOFT_FIXED_THE_DXGI_RUNTIME_AND_IT_NO_LONGER_CACHES_HDR_CAPS_IMMUTABLY
    bool bHDRCapable =
      rb.isHDRCapable ();

    if (rb.checkHDRState () != bHDRCapable)
    {
      SK_LOG0 ( ( L"Display HDR Capabilities Changed..." ),
                  L"WindowsHDR" );
    }
#endif
  ...
  switch (uMsg)
  {
    ...
    case WM_SETFOCUS:
      if ((! IsWindow (game_window.hWnd)) && IsWindow (hWnd))
